...
def get_main_domain(url: str) -> str:
    """
    Extracts main domain without subdomains.

    Args:
        url: The URL to extract main domain from

    Returns:
        Main domain (e.g., en.wikipedia.org -> wikipedia.com)
    """
    parsed = urlparse(url)
...
def generate_slug(url: str, title: str | None = None) -> str:
    ...
    query = parsed.query  # Store query for later use
